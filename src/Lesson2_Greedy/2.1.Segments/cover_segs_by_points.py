def segs_cover(x: list) -> list:
    """
    сортирую отрезки по правому концу, 
    затем в результирующий массив присваиваю конец первого отрезка,
     потом сравниваю лежит ли эта точка на след. отрезке,
      если да то ничего не делаю и перехожу к след. отрезку(сравниваю эту же точку но уже со след. отрезком)#continue!!#,
      если нет, то в результирующий массив присваиваю конечную точку этого отрезка 
     и так до конца.
     (delete after coding!)
